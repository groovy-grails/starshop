...
<div class="fieldcontain ${hasErrors(bean: nideshopGoodsInstance, field: 'primaryProductId', 'error')} ">
	<label for="primaryProductId">
		<g:message code="nideshopGoods.primaryProductId.label" default="Primary Product Id" />
		
	</label>
	<g:select id="primaryProductId" name="primaryProductId.id" from="${nideshop.NideshopGoods.list()}" optionKey="id" value="${nideshopGoodsInstance?.primaryProductId?.id}" class="many-to-one" noSelection="['null': '']"/>
